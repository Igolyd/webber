...
export const useUsersStore = defineStore("users", () => {
  ...
  const getByUniqueName = (uniqueName: string) =>
    users.value.find(
      (u) => u.uniqueName.toLowerCase() === uniqueName.toLowerCase()
    ) || null;
  ...
  const searchUsers = (q: string, excludeIds: string[] = []) => {
    ...
    return users.value.filter((u) => {
      ...
      return (
        ...
        u.uniqueName.toLowerCase().includes(s)
      );
    });
  };
  ...
  // ACTIONS
  function addUser(
    payload: Partial<AppUser> & { name: string; uniqueName?: string }
  ) {
    const now = new Date().toISOString();
    let uniqueName = payload.uniqueName
      ? slugifyUniqueName(payload.uniqueName)
      : slugifyUniqueName(payload.name);
  ...
  function ensureUserByAccount(payload: {
    userId?: string;
    name?: string;
    avatar?: string;
    uniqueName?: string;
  ...
  function patchUser(
    userId: string,
    partial: Partial<Omit<AppUser, "id" | "createdAt">>
  ) {
  ...
  function addUserToGroup(userId: string, groupId: string) {
    ...
    if (!u.memberships.some((m) => m.groupId === groupId)) {
      const updated = {
        ...u,
        memberships: [...u.memberships, { groupId, roleIds: [] }],
      };
      users.value = users.value.map((x) => (x.id === userId ? updated : x));
    }
  }
  ...
  function removeUserFromGroup(userId: string, groupId: string) {
    ...
    const updated = {
      ...u,
      memberships: u.memberships.filter((m) => m.groupId !== groupId),
    };
    users.value = users.value.map((x) => (x.id === userId ? updated : x));
  }
  ...
  function assignRole(userId: string, groupId: string, roleId: string) {
    ...
    if (!mem) {
      updated = {
        ...u,
        memberships: [...u.memberships, { groupId, roleIds: [roleId] }],
      };
  ...
  function addFriend(userId: string, friendId: string, reciprocal = true) {
    ...
    const newU = u.friendIds.includes(friendId)
      ? u
      : { ...u, friendIds: [...u.friendIds, friendId] };
    let newF = f;
    ...
    users.value = users.value.map((x) =>
      x.id === newU.id ? newU : x.id === newF.id ? newF : x
    );
  }
  ...
  function removeFriend(userId: string, friendId: string, reciprocal = true) {
    ...
    const newU = {
      ...u,
      friendIds: u.friendIds.filter((id) => id !== friendId),
    };
    let newF = f;
    if (f && reciprocal) {
    ...
    users.value = users.value.map((x) =>
      x.id === userId ? newU : x.id === friendId ? (newF as AppUser) : x
    );
  }
