  ...
  <v-card class="profile-card" elevation="10">
    ...
    <v-card-text class="pt-1 px-4">
      ...
      <div class="mt-2 d-flex justify-center">
        <v-chip size="small" variant="tonal" prepend-icon="mdi-music">
          {{ groupTag || 'Без группы' }}
        </v-chip>
      </div>
    ...
    <v-card-actions class="px-4 pt-2">
      <div class="w-100 d-flex ga-2 flex-wrap">
        <div class="activator-wrap">
          ...
          <v-menu v-model="openStatusMenu" activator="parent" location="bottom" :close-on-content-click="true">
            <v-list density="compact" style="min-width: 220px">
              ...
              <v-list-item v-for="s in statuses" :key="s.value" @click="setStatus(s.value)">
                ...
                <template #append>
                  <v-icon v-if="status === s.value" color="primary">mdi-check</v-icon>
                </template>
              </v-list-item>
            </v-list>
          </v-menu>
...
export default defineComponent({
  ...
  setup() {
    ...
    const {
      name, avatar, banner, bannerColor, bannerOverlayColor, bannerOverlayOpacity,
      about, quote, groupTag, status
    ...
    const statuses = [
      { value: 'online',    label: 'В сети',        icon: 'mdi-circle',         color: '#3fb950' },
      { value: 'idle',      label: 'Неактивен',     icon: 'mdi-circle',         color: '#f2c94c' },
      { value: 'dnd',       label: 'Не беспокоить', icon: 'mdi-minus-circle',   color: '#f44336' },
      { value: 'invisible', label: 'Невидимка',     icon: 'mdi-circle-outline', color: '#9aa0a6' },
    ] as const
    const statusMeta = computed(() => statuses.find(s => s.value === status.value as StatusVal)!)
    ...
    function onEditProfile() {/* роутинг сделаешь сам */}
    function onSwitchAccounts() {/* no-op */}

    return {
      avatar, banner, bannerColor, bannerOverlayColor, bannerOverlayOpacity,
      about, quote, groupTag, status,
      userId, displayName,
      btnDensity, avatarSize, defaultAvatar, defaultBanner,
      statuses, statusLabel, statusIcon, openStatusMenu, setStatus,
      copiedSnack, copyUserId,
      onEditProfile, onSwitchAccounts,
    }
  },
})
</script>
