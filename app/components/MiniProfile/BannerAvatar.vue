...
function isUrlLike(v: string) {
  return (
    /^(https?:|data:|blob:)/i.test(v) || /.(png|jpe?g|gif|webp|svg)$/i.test(v)
  );
}
...
const bannerBoxStyle = computed(() => {
  ...
  if (raw && (raw.startsWith("url(") || isUrlLike(raw))) {
    const urlStr = raw.startsWith("url(") ? raw : `url("${raw}")`;
    return {
  ...
  const base =
    raw || props.bannerColor || "linear-gradient(180deg, #8ec5e5, #4f9cf9)";
