<template>
  <div class="pa-4 scope-main">
    ...
    <div class="text-caption text-medium-emphasis mb-2">
      meId: {{ meId }} | ownIds:
      {{ (reactions.state?.userOwnSticker?.[meId] || []).length }}
    </div>
    ...
    <h3 class="text-subtitle-2 mt-4 mb-2">Импортированные</h3>
    <div class="d-flex flex-wrap gap-1">
      <v-chip
        v-for="uname in importedSticker"
        :key="uname"
        class="ma-1"
        size="small"
        closable
        @click:close="onRemoveImport(uname)"
      >
        {{ uname }}
      </v-chip>
      <div v-if="!importedSticker.length" class="text-medium-emphasis">Нет</div>
    </div>
  </div>
...
import { useReactionsStore, type StickerPack } from "@/stores/reactions";
...
watch(meId, (v) => console.debug("[StickerScreen] meId changed", v), {
  immediate: true,
});
...
const ownPacks = computed(() => {
  ...
  const ids = uid ? reactions.state.userOwnSticker?.[uid] ?? [] : [];
  const res = ids
    .map((id: string) => reactions.getPackById("sticker", id))
    .filter((p): p is StickerPack => Boolean(p));
  console.debug("[StickerScreen] ownPacks", {
    uid,
    idsCount: ids.length,
    resCount: res.length,
  });
...
const importedSticker = computed<string[]>(
  () => reactions.state.userImportedSticker?.[meId.value] ?? []
);
...
const limits = {
  ...
  USER_OWN_STICKER_PACKS_LIMIT: reactions.USER_OWN_STICKER_PACKS_LIMIT,
};
...
function onRemoveImport(uname: string) {
  if (!meId.value) return;
  reactions.removeImportFromUser(meId.value, "sticker", uname);
}
function onEditorChanged() {
  // auto recomputed
}
onMounted(() => {
  console.debug("[StickerScreen] mounted", {
    meId: meId.value,
    ownIds: reactions.state.userOwnSticker?.[meId.value] ?? [],
  });
});
</script>
<style scoped>
