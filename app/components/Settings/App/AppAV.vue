  ...
  <v-container>
    ...
    <v-window v-model="tab" class="mt-4">
      <v-window-item value="audio">
        <v-card>
          <v-card-text>
            ...
            <v-row class="mb-2">
              <v-col cols="12" md="6">
                <v-slider v-model="av.inputVolume" min="0" max="200" step="1" label="Громкость ввода (%)" />
              </v-col>
              <v-col cols="12" md="6">
                <v-slider v-model="av.outputVolume" min="0" max="200" step="1" label="Громкость вывода (%)" />
              </v-col>
            </v-row>

            <v-row class="mb-2">
              <v-col cols="12" md="8">
                <div class="text-subtitle-2">Чувствительность микрофона (порог, дБ)</div>
                <v-slider v-model="av.inputSensitivity" min="-80" max="-10" step="1" />
                <div class="text-caption">Звук ниже порога будет игнорироваться (noise gate).</div>
              </v-col>
            </v-row>

            <v-row align="center" class="mb-2 ga-4">
              <v-btn color="primary" @click="toggleTest">{{ testing ? 'Остановить проверку' : 'Проверка' }}</v-btn>
              <div class="d-flex align-center ga-2">
                <div style="width:240px">
                  <v-progress-linear :model-value="meterValue" height="16" color="green" />
                </div>
      ...
      <v-window-item value="video">
        <v-card>
          <v-card-text>
            ...
            <video ref="videoEl" autoplay playsinline style="max-width:100%; border-radius:8px;"></video>
          </v-card-text>
        </v-card>
      </v-window-item>
      ...
      <v-window-item value="soundbar">
        <v-card>
          <v-card-text>
            Здесь может быть микшер источников/уведомлений, баланс, тест воспроизведения и т.п. (пока заглушка).
          </v-card-text>
        </v-card>
      </v-window-item>
    </v-window>
...
export default defineComponent({
  setup() {
    ...
    const { stream: videoStream, start: startVideo, stop: stopVideo, constraints } = useUserMedia({
      video: true,
      audio: false,
    })
    ...
    async function startMicTest() {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          audio: { deviceId: av.inputDeviceId ? { exact: av.inputDeviceId } : undefined },
          video: false,
        })
    ...
    watch(() => av.inputVolume, (v) => { if (gainNode) gainNode.gain.value = v / 100 })
    watch(() => av.outputVolume, (v) => { if (monitor.value) monitor.value.volume = Math.min(v / 100, 2) })
    ...
    onMounted(async () => {
      // Инициализируем текущие потоки (необязательно стартовать сразу)
      await navigator.mediaDevices.getUserMedia({ audio: true, video: true }).catch(()=>{})
      // Установим дефолтные камеры/мики если нет
      if (!av.inputDeviceId && audioInputs.value.length) av.inputDeviceId = audioInputs.value[0].deviceId
      if (!cameraDeviceId.value && videoInputs.value.length) cameraDeviceId.value = videoInputs.value[0].deviceId
    })
    ...
    return {
      ...
      cameraDeviceId, videoEl, monitor,
      testing, meterValue, toggleTest,
    }
  },
})
</script>