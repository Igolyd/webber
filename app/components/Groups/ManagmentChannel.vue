          ...
          >
            <template #item="{ props, item }">
              <v-list-item v-bind="props">
                <template #prepend>
                  <v-icon size="18">{{ item.raw.value ? 'mdi-folder-outline' : 'mdi-folder-off-outline' }}</v-icon>
                </template>
...
const props = defineProps<{
  ...
  channel?: { id: string; name: string; type: 'text' | 'voice'; directoryId: string | null }
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', v: boolean): void
  (e: 'submit', payload: { id?: string; name: string; type: 'text' | 'voice'; directoryId: string | null }): void
  (e: 'cancel'): void
}>()
