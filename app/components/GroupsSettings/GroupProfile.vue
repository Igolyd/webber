          ...
          <v-row>
            ...
            <v-col cols="12" md="6">
              <FormKit
                type="text"
                name="pronouns"
                label="Местоимение/формулировка"
            ...
            <v-col cols="12" md="6">
              ...
              <div class="d-flex ga-2">
                ...
                <v-btn size="small" variant="text" v-if="profiles.decoration">
                  Удалить
                </v-btn>
              </div>
            </v-col>
...
// ВАЖНО: проверьте путь и название стора под ваш проект.
// Ожидается совместимый API с user/profiles: поля и методы ниже.
import { useGroupsStore } from "~/stores/groups";

const profiles = useGroupsStore();
...
// Можно заменить на отдельный дефолт для группы
const defaultAvatar = "/app/assets/profile/profile_exp.jpg";

// Источник аватара для превью
const currentAvatarSrc = computed(
  () => previewImage.value || profiles.avatar || defaultAvatar
);

// Синхронизация локального превью с хранилищем
watch(
  () => profiles.avatar,
  (val) => {
...
function handleFileChange(e: Event) {
  ...
  // GIF: сразу сохраняем как dataURL, без кадрирования
