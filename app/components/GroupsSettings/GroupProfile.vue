...
function openBadgePicker() {
  ...
  if (isGradientLike(current)) {
    ...
    if (m) { badgeAngle.value = parseInt(m[1], 10) || 135; badgeC1.value = m[2].trim(); badgeC2.value = m[3].trim() }
  } else {
...
function saveBadge() {
  ...
  const nextBadge = { id: prev?.id || 'custom', name: prev?.name || 'Пользовательский бейдж', icon: prev?.icon, color: nextColor }
...
function openBannerPicker() {
  ...
  if (isGradientLike(current)) {
    ...
    const m = current.match(/linear-gradient(\d+)deg,\s*([']+),\s*([)])/i)
    if (m) { bannerAngle.value = parseInt(m[1], 10) || 135; bannerC1.value = m[2].trim(); bannerC2.value = m[3].trim() }
  } else {
...
const bannerPreviewStyle = computed(() => {
  ...
  const base: Record<string, string> = { border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px' }
